...
public class FirstPersonDrawer extends DefaultViewer {
	...
	private void drawBackground(MazePanel panel) {
		...
		panel.setColorToBlack();
		//gc.fillRect(0, 0, view_width, view_height/2);
		panel.MazeFillRect(0,0,view_width, view_height/2);
		//gc.setColor(Color.darkGray);

		panel.setColorToDarkGray();
		//gc.fillRect(0, view_height/2, view_width, view_height/2);
		panel.MazeFillRect(0, view_height/2, view_width, view_height/2);
	}
	/**
	...
	private boolean boundingBoxIsVisible(BSPNode b) {
		...
		int[] p = {x1, x2};
		//panel.rsetIntersectCreatePoint(x1,x2,rset);
		return (rset.intersect(p));
		//return(panel.rsetIntersectCreatePoint(x1,x2,rset));
	}
	...
	private void drawSegment(Seg seg) {
		...
		while (x1i <= x2) {
			...
			if (!rset.intersect(p))
				break;
			//int[] pointXval = panel.getPointXvalfromCreated(x1i, x2);
			//x1i = p.x;
			//int x2i = p.y;
			x1i = p[0];
			int x2i =p[1];
			...
			panel.MazeFillPolygon(xps, yps, 4);
